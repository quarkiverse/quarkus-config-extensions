...
This guide explains how your Quarkus application can read configuration properties at runtime from https://www.consul.io[HashiCorp Consul].
...
This Key-Value store is what the `quarkus-config-consul` extension interacts with in order to allow Quarkus applications to read runtime configuration properties from Consul.
...
If you already have your Quarkus project configured, you can add the `config-consul` extension
...
include::./includes/quarkus-consul-config.adoc[]